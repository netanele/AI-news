...
class TestFetchVideos:
    ...
    def test_returns_recent_videos(self, mock_parse):
    ...
    @patch("pipeline.rss_fetcher.feedparser.parse")
    def test_filters_old_videos(self, mock_parse):
        now = datetime.now(timezone.utc)
        feed = MagicMock()
        feed.bozo = False
        feed.entries = [
    ...
    @patch("pipeline.rss_fetcher.feedparser.parse")
    def test_channel_error_skipped(self, mock_parse):
        mock_parse.side_effect = Exception("Network error")
        result = fetch_videos([_make_channel()], days_to_show=7)
        assert len(result) == 0

    @patch("pipeline.rss_fetcher.feedparser.parse")
    def test_video_metadata_fields(self, mock_parse):
        now = datetime.now(timezone.utc)
        feed = MagicMock()
        feed.bozo = False
    ...
    @patch("pipeline.rss_fetcher.feedparser.parse")
    def test_multiple_channels(self, mock_parse):
        now = datetime.now(timezone.utc)
        feed = MagicMock()
        feed.bozo = False
